(function() {
	...
	test('Parsing comments', function() {
		...
		// "<!--->" is parsed as a comment without value by the browser.
		counter = createCounter(writer);
		parser = new tinymce.html.SaxParser(counter, schema);
		writer.reset();
		parser.parse('<!--->');
		equal(writer.getContent(), '', 'Parse "<!--->" as a comment without value.');
		deepEqual(counter.counts, {}, 'Parse "<!--->" as a comment without value count.');
